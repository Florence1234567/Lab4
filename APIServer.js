...
import * as CachedRequestsManager from "./models/CachedRequestsManager.js";
...
export default class APIServer {
  ...
  showRequestInfo() {
    ...
    log(
      FgGreen,
      Bright,
      `Request --> [${this.httpContext.req.method}::${this.httpContext.req.url}]`
    );
    log("User agent ", this.httpContext.req.headers["user-agent"]);
    log(
      "Host ",
      this.httpContext.hostIp.substring(0, 15),
      "::",
      this.httpContext.host
    );
    if (this.httpContext.payload)
      log(
        "Request payload ",
        JSON.stringify(this.httpContext.payload).substring(0, 127) + "..."
      );
  }
  ...
  showRequestProcessTime() {
    ...
    log(
      ...
      "Response time: ",
      Math.round(
        ((requestProcessEndTime[0] * 1000 +
          requestProcessEndTime[1] / 1000000) /
          1000) *
          10000
      ) / 10000,
      "seconds"
